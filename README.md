...
- Trains in **two phases**: Phase 1 freezes the backbone and trains only the heads; Phase 2 unfreezes the top layers of ResNet50 and fine-tunes everything at a very low learning rate
...
- If `color_aux_v1.csv` exists, merges it into training as **colour-only rows** — since these rows have no season/usage ground truth, their season/usage loss is masked out (weight 0) via a `has_full_labels` flag, so they only ever train the colour head. Only merged into training, never into validation/test, so evaluation numbers stay directly comparable across runs
